...
| M | 内容 | 完了の目安 |
|---|---|---|
| **G1a** 作成 ✅ | 全 <!--g1:requirements-->69<!--/g1--> 要件を原文の節末まで読み、<!--g1:obligations-->337<!--/g1--> 義務に分解。`tests/{specs,coverage,predicates}.yaml` と `docs/04`（生成物）を作成 | **完了（PENDING_REVIEW）** |
...
| **G2** テスト設計 ⏳ | <!--g1:case_target-->336<!--/g1--> 義務を**ケース ID に割り当て**、`required_variants` の網羅と positive/negative control を定義。**作成者以外が設計をレビュー**（[G2 の詳細](#-設計ゲート-g2--テスト設計)） | 「義務は正しいがケースに検出力がない」を防ぐ |
...
`coverage.yaml` の <!--g1:obligations-->337<!--/g1--> 義務のうち、`NOT_OBSERVABLE`（<!--g1:not_observable_keys-->`IIP-SSO05.a4`<!--/g1-->）を除く **<!--g1:case_target-->336<!--/g1--> 義務**。
...
| `ATTESTED` | <!--g1:tb_attested-->44<!--/g1--> | 対象内部の挙動を申告 |
...
- [ ] **<!--g1:case_target-->336<!--/g1--> 義務すべてが 1 件以上のケースに割り当てられている**（CI で検証）
