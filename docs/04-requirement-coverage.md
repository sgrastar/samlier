...
| `IIP-SSO01.fh` | SHOULD | sp | `ATTESTED` | — | full | 応答元の能力が不明な場合、応答元は要求元が対応する最高版に対応していると仮定することが望ましい |
| `IIP-SSO01.er` | MUST | idp/sp | `AUTOMATED` | `target_signs_saml_messages`<br>(CAPABILITY_BASED) | core | assertion とプロトコルメッセージの XML 署名は enveloped でなければならない |
...
| `IIP-SSO01.eu` | MUST | idp/sp | `AUTOMATED` | `target_signs_saml_messages`<br>(CAPABILITY_BASED) | core | 対象が署名する assertion / プロトコルメッセージのルート要素に ID 属性の値を与える |
| `IIP-SSO01.ev` | MUST | idp/sp | `AUTOMATED` | `target_signs_saml_messages`<br>(CAPABILITY_BASED) | core | 署名は単一の <ds:Reference> を含み、その URI は署名対象ルート要素の ID への same-document reference である |
| `IIP-SSO01.ew` | SHOULD | idp/sp | `AUTOMATED` | `target_signs_saml_messages`<br>(CAPABILITY_BASED) | full | <ds:CanonicalizationMethod> と <ds:Transform> の双方で Exclusive Canonicalization を使うことが望ましい |
| `IIP-SSO01.ex` | SHOULD_NOT | idp/sp | `AUTOMATED` | `target_signs_saml_messages`<br>(CAPABILITY_BASED) | full | 署名に enveloped-signature / exclusive c14n 以外の transform を含めるべきでない |
| `IIP-SSO01.ey` | MUST | idp/sp | `BROWSER` | `accepts_nonstandard_signature_transforms`<br>(CAPABILITY_BASED) | core | 許可外の transform を含む署名を拒否しない場合、SAML メッセージのどの内容も署名対象から除外されていないことを保証する |
...
- **対照（negative control）**:
  ...
  - ★ 条件は『許可外 transform を含む署名を一律拒否しない場合』。一律拒否する対象では NOT_APPLICABLE であり、それが安全側の挙動である。条件述語は本検査そのものの観測（許可外 transform 付き署名を送って受理されたか）から決まる
...
- **必要な variant**:
  - `v-b78ab58b39` 対象が http://www.w3.org/2000/09/xmldsig#rsa-sha1 の署名検証に対応していることを申告で確認する
  - `v-fe30dc59b8` 対象が同アルゴリズムでの署名生成に対応していることを申告で確認する
  - `v-94200fcce5` IIP-ALG08.a により当該アルゴリズムを**禁止に設定している**場合、その旨を申告で確認する
- **対照（negative control）**:
  - ★ SHOULD_CLASS。対応していなければ WARNING であって FAIL ではない
  - ★ 原文照合表から無言で落とさないために義務として残しているが、SHA-1 は現在では危殆化しており、IIP-ALG02.a は RSA-SHA256 を MUST としている。また IIP-ALG08.a は『特定アルゴリズムの使用を禁止でき、設定・選択の試行が失敗するようにできる』ことをMUST としており、**rsa-sha1 を禁止する配備は IIP が明示的に認めた設定選択**である
  - ★ したがって『rsa-sha1 を禁止しているため本 SHOULD を満たさない』場合、Evaluator は WARNING を出すが、結果には上記の理由を advisory として併記する。G2 ではこのケースを control_waiver_ja に記録し、mutant 検出力の評価から外す
  - ★ 判定は能力の申告で行う。禁止設定を解除させてまで検査しない
- **参照先仕様**: `SAML2Prof#4.1`
- **source_clauses**: `[0, 125)` `sha256:ff1057626aaa…`
- **review**: `PENDING_REVIEW` / reviewer: `None` / approved_at: `None`
...
- **対照（negative control）**:
  ...
  - ★ 条件は『署名する <Assertion> を暗号化する場合』。assertion 署名（IIP-SSO04）も assertion 暗号化（IIP-IDP09.a）も IIP では対応必須なので構成できるはずだが、両方を同時に有効にできない場合は not_verified
...
- **必要な variant**:
  - `v-6629ae9a3f` 属性暗号化と assertion 署名を両方有効にした構成 → 署名の対象に <saml:EncryptedAttribute> が含まれる
