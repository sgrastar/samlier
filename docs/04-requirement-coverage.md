...
- **必要な variant**:
  - `v-a4cf17b0cb` 観測した <saml:EncryptedID> ごとに、包含する <Assertion> **または** <Response> の**少なくとも一方**に有効な署名があり、その署名が当該 <EncryptedID> を覆っている
  - `v-165f35988d` その署名について (a) 元の送出文書の署名検証が**成功する** かつ (b) <EncryptedID> を差し替えた文書の署名検証が**失敗する**
...
- **対照（negative control）**:
  - ★ 原文の署名対象は『the assertion **or message** containing the encrypted element』であり、**選言（or）**である。assertion 署名と <Response> 署名を**別々の required variant にすると、G2 では両方を覆う必要が生じ、実質的に and を要求してしまう**。訂正して『少なくとも一方に有効な署名がある』という単一の規則にし、両方の検査は実際に両方の署名が観測された場合に限るようにした
...
- **必要な variant**:
  - `v-8c3322d455` 観測した <saml:EncryptedAttribute> ごとに、包含する <Assertion> **または** <Response> の**少なくとも一方**に有効な署名があり、その署名が当該 <EncryptedAttribute> を覆っている
  - `v-f3ddf5d9e0` その署名について (a) 元の送出文書の署名検証が**成功する** かつ (b) <EncryptedAttribute> を差し替えた文書の署名検証が**失敗する**
...
- **対照（negative control）**:
  - ★ 原文の署名対象は『the assertion **or message** containing the encrypted element』であり選言（or）。別々の required variant にすると G2 で両方を覆う必要が生じ、実質的に and を要求してしまうので、『少なくとも一方に有効な署名がある』という単一の規則にした
  - ★ 受動規則として扱う。対象が実際に送出した該当要素ごとに検査し、当該 Run で 1 件も観測されなければ satisfied_with_note（観測機会なし）とする。**条件述語を置かない**: 肯定的な観測材料しか作れない CAPABILITY_BASED 述語では『機能を持たない』ことを FALSE にできず（申告のみの false は UNKNOWN）、非対応の対象が永久に not_verified になる。IIP-SSO01.er 等と同じ扱いに揃えた
