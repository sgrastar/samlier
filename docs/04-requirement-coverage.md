...
| `IIP-SSO01.dp` | MUST | idp | `AUTOMATED` | `supports_encrypted_nameid`<br>(CAPABILITY_BASED) | core | <saml:EncryptedID> の暗号文は、暗号化操作ごとに一意である |
...
| `IIP-SSO01.fd` | MUST | idp | `CONFIG` | `supports_encrypted_nameid`<br>(CAPABILITY_BASED) | core | <BaseID> / <NameID> を暗号化する場合、暗号データは平文と同じ位置に置き換わる |
| `IIP-SSO01.fe` | MUST | idp | `CONFIG` | `supports_encrypted_attribute`<br>(CAPABILITY_BASED) | core | <Attribute> を暗号化する場合、暗号データは平文と同じ位置に置き換わる |
| `IIP-SSO01.fa` | MUST | sp | `ATTESTED` | — | core | 依拠当事者は、署名と暗号化が行われた順序と逆の順序で、署名検証と復号を行う |
| `IIP-SSO01.fb` | MUST | idp | `CONFIG` | `signs_and_encrypts_assertion`<br>(CAPABILITY_BASED) | core | 署名済みの <Assertion> を暗号化する場合、署名を先に計算して <Assertion> 内に置いてから暗号化する |
| `IIP-SSO01.fc` | MUST | idp | `CONFIG` | `supports_encrypted_nameid`<br>(CAPABILITY_BASED) | core | <BaseID> / <NameID> を暗号化する場合、先に暗号化し、その後で暗号化要素を含む assertion / メッセージに署名する |
| `IIP-SSO01.ff` | MUST | idp | `CONFIG` | `supports_encrypted_attribute`<br>(CAPABILITY_BASED) | core | <Attribute> を暗号化する場合、先に暗号化し、その後で暗号化要素を含む assertion / メッセージに署名する |
...
- **対照（negative control）**:
  - ★ 受動的な常時チェック
  - ★ 条件は『XML 署名を付ける場合』。署名しない対象では NOT_APPLICABLE
...
- **必要な variant**:
  - `v-b6908ef8bf` 【二択評価】XPath transform で assertion の一部（AttributeStatement 等）を署名対象から除外した応答 → **拒否される、または（受理するなら）除外された内容が処理に使われていない**
  - `v-c859ec8127` 【二択評価】XSLT transform を含む署名 → 同上
  - `v-d3718f6f56` 【二択評価】署名対象を空にする transform を含む署名 → 同上
  - `v-c44316ff85` 【二択評価】<ds:Reference>/@URI を空（文書全体）にしたうえで XPath で一部を除外した署名 → 同上
  - `v-a3ae670ae7` 対照: 許可された transform だけの正しい署名 → 受理される
- **対照（negative control）**:
  - ★ **XML Signature Wrapping の中核的な防御**。P0 相当の検出力を持たせる
  - ★ 訂正: 前版は述語 accepts_nonstandard_signature_transforms を条件にしていたが、**適用性はケース実行より先に評価される**（docs/03 §条件の評価）ため、観測するためのケースが観測前にスキップされるという循環になっていた。さらに、1 種類の transform を拒否した観測だけで条件を偽にすると、**別の危険な transform を受理する実装を NOT_APPLICABLE として除外できてしまう**。条件を外し、**各 variant を transform ごとに二択で評価する**形にした
  - ★ 二択の評価規則: transform ごとに「拒否した」→ satisfied ／「受理したが除外された内容が処理に使われていない」→ satisfied ／「受理して除外された内容が処理に使われた（または除外の有無を確認していない）」→ violated
  ...
  - ★ 受理したのに『除外の有無を確認していない』ことは実装内部の話なので、外形的には『除外された内容が処理に使われたか』で代替判定する。除外内容が使われていないことを確認できなければ not_verified
- **参照先仕様**: `SAML2Prof#4.1`
- **source_clauses**: `[0, 125)` `sha256:ff1057626aaa…`
- **review**: `PENDING_REVIEW` / reviewer: `None` / approved_at: `None`
...
- **対照（negative control）**:
  - ★ IIP-IDP09.b により識別子の暗号化は OPTIONAL。非対応なら条件が偽で NOT_APPLICABLE
- **設定不能時の意味**: `test_precondition`
- **参照先仕様**: `SAML2Prof#4.1`
- **source_clauses**: `[0, 125)` `sha256:ff1057626aaa…`
- **review**: `PENDING_REVIEW` / reviewer: `None` / approved_at: `None`
...
- **対照（negative control）**:
  - ★ IIP-IDP09.b により属性の暗号化は OPTIONAL。非対応なら条件が偽で NOT_APPLICABLE
- **設定不能時の意味**: `test_precondition`
- **参照先仕様**: `SAML2Prof#4.1`
- **source_clauses**: `[0, 125)` `sha256:ff1057626aaa…`
- **review**: `PENDING_REVIEW` / reviewer: `None` / approved_at: `None`
...
- **対照（negative control）**:
  ...
  - ★ 訂正: 前版は無条件だった。IIP-SSO04（assertion 署名）と IIP-IDP09.a（assertion 暗号化）は**それぞれ独立した対応必須要件**であって、両方を同一 assertion に同時適用できることまでは導けない。Core §6.2 も『署名する Assertion を暗号化する場合』の順序を定めるだけで組合せ能力は要求していない。別々には対応するが同時構成を提供しない実装が永久に not_verified になっていた。述語 signs_and_encrypts_assertion を条件にした
  - ★ 条件の観測は Test Plan の構成段階（preflight / WAITING_CONFIG）で得る。本義務のケースが観測源ではない
...
- **対照（negative control）**:
  ...
  - ★ 差し替えテストが検出力の要。署名が平文時点で計算されていると、暗号文の差し替えが検出できてしまう（＝署名が暗号文を覆っていない）
  - ★ 条件は『識別子を暗号化する場合』。IIP-IDP09.b により OPTIONAL なので、非対応なら NOT_APPLICABLE
...
- **必要な variant**:
  ...
  - `v-c08cfd22da` 署名を検証したうえで <EncryptedAttribute> を差し替えると検証が失敗する
...
- **対照（negative control）**:
  ...
  - ★ 条件は『属性を暗号化する場合』。IIP-IDP09.b により OPTIONAL なので、非対応なら NOT_APPLICABLE
  - ★ IIP-SSO01.fc（識別子）と同じ規則を属性に適用したもの。暗号化する要素の種類ごとに対応状況が違うので義務を分けた
- **設定不能時の意味**: `test_precondition`
- **参照先仕様**: `SAML2Prof#4.1`
- **source_clauses**: `[0, 125)` `sha256:ff1057626aaa…`
- **review**: `PENDING_REVIEW` / reviewer: `None` / approved_at: `None`
