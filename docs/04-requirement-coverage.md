...
| `IIP-SSO01.fc` | MUST | idp | `CONFIG` | — | core | <BaseID> / <NameID> を暗号化する場合、先に暗号化し、その後で暗号化要素を含む assertion / メッセージに署名する |
| `IIP-SSO01.ff` | MUST | idp | `CONFIG` | — | core | <Attribute> を暗号化する場合、先に暗号化し、その後で暗号化要素を含む assertion / メッセージに署名する |
...
- **対照（negative control）**:
  ...
  - ★ 訂正 2: 前版は『許可外 transform を含むが内容を一切除外していない署名（恒等な XPath）→ 受理してよい』をrequired variant に置いていたが、原文は**内容を除外していなくても拒否してよい**としている（MAY）。受理・拒否の二択であり verdict を付けられない。Suite 側 fixture の自己検証（『拒否は transform の存在ではなく除外の検出によるか』の確認）に移し、**対象の verdict には影響させない**
  - ★ 評価規則:
...
- **対照（negative control）**:
  ...
  - ★ 恒等な transform のケースは Suite 側 fixture の自己検証に置き、対象の verdict には影響させない
  - ★ 未署名 AuthnRequest を受理する構成では観測機会がない。署名済み要求を送れる Test Plan でのみ実行する
- **参照先仕様**: `SAML2Prof#4.1`
- **source_clauses**: `[0, 125)` `sha256:ff1057626aaa…`
- **review**: `PENDING_REVIEW` / reviewer: `None` / approved_at: `None`
...
- **必要な variant**:
  - `v-e01dcabd81` 【assertion 署名の経路】識別子暗号化と assertion 署名を有効にした構成 → assertion の署名対象に <saml:EncryptedID> が含まれる
  - `v-811d503212` 【<Response> 署名の経路】識別子暗号化と <Response> 署名を有効にした構成 → <Response> の署名対象に <saml:EncryptedID> が含まれる（原文の『the assertion **or message** containing the encrypted element』）
  - `v-a2520d1f03` 【negative】<saml:EncryptedID> を送出しながら、包含する <Assertion> にも <Response> にも**有効な署名がない** → violated
  - `v-b5f546332b` 検証の組: (a) 元の送出文書の署名検証が**成功する** かつ (b) <EncryptedID> を差し替えた文書の署名検証が**失敗する**
  - `v-b795024638` 両方の署名経路を提供する対象では、両方を検査する
- **対照（negative control）**:
  - ★ 訂正: 前版は assertion 署名との組合せしか認めていなかったが、原文の署名対象は『the assertion **or message** containing the encrypted element』である。暗号化後の <EncryptedID> を含む <Response> 全体を署名する方式も適合する
  ...
  - ★ **暗号化要素を観測したのに包含する要素に有効な署名がない場合は『観測機会なし』ではない**。satisfied_with_note ではなく violated である（原文は暗号化後に署名することまで求めている）
...
- **必要な variant**:
  - `v-b2ae6102e8` 【assertion 署名の経路】属性暗号化と assertion 署名を有効にした構成 → assertion の署名対象に <saml:EncryptedAttribute> が含まれる
  - `v-82fa785a34` 【<Response> 署名の経路】属性暗号化と <Response> 署名を有効にした構成 → <Response> の署名対象に <saml:EncryptedAttribute> が含まれる
  - `v-19c8abf37d` 【negative】<saml:EncryptedAttribute> を送出しながら、包含する <Assertion> にも <Response> にも**有効な署名がない** → violated
  - `v-965d95fd30` 検証の組: (a) 元の送出文書の署名検証が**成功する** かつ (b) <EncryptedAttribute> を差し替えた文書の署名検証が**失敗する**
  - `v-21ffa80a29` 両方の署名経路を提供する対象では、両方を検査する
- **対照（negative control）**:
  - ★ 訂正: 前版は assertion 署名との組合せしか認めていなかった。原文の署名対象は『the assertion **or message** containing the encrypted element』
  - ★ 受動規則として扱う。対象が実際に送出した該当要素ごとに検査し、当該 Run で 1 件も観測されなければ satisfied_with_note（観測機会なし）とする。**条件述語を置かない**: 肯定的な観測材料しか作れない CAPABILITY_BASED 述語では『機能を持たない』ことを FALSE にできず（申告のみの false は UNKNOWN）、非対応の対象が永久に not_verified になる。IIP-SSO01.er 等と同じ扱いに揃えた
  - ★ IIP-IDP09.b により属性の暗号化は OPTIONAL。暗号化しない対象では観測されず satisfied_with_note になる
  - ★ **暗号化要素を観測したのに包含する要素に有効な署名がない場合は『観測機会なし』ではない**。satisfied_with_note ではなく violated である
